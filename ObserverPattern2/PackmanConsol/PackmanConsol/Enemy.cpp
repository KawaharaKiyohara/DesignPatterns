...
Enemy::Enemy()
{
	//課題ヒント　この部分をポリモーフィズムを活用するコードに変更する。
	//プレイヤーにリスナーを登録する。
	g_player.AddStateListener(this);
	g_player2.AddStateListener(this);
}
...
Enemy::~Enemy()
{
	...
	g_player2.RemoveStateListener(this);
}
...
void Enemy::Update()
{
	if (m_state == State_Search) {
		...
		CVector2 playerPos = {0.0f, 0.0f};
		if (g_player.IsPowerUp()) {
			playerPos = g_player.GetPosition();
		}
		else if (g_player2.IsPowerUp()) {
			playerPos = g_player2.GetPosition();
		}
